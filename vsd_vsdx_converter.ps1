...
#
	...
	if (-not ($sourceDirectory.Trim())) {
		...
		try {
			...
			Write-Host "Unable to open MS Visio."
			Write-Host $_
		...
		foreach ($file in $vsdFiles) {
			...
			if (Test-Path -Path $vsdxFile -PathType Leaf) {
				...
				Write-Host "`nSkipped conversion for [$vsdxFile] because a file with that name already exists."
			}
			else {
				# Open the VSD
				...
				$convertedFiles++
				Write-Host "`nConverted $vsdFile `nto $vsdxFile"
			}
		}

		# Quit Visio
		$visio.Quit()
		Write-Host "`nConverted $convertedFiles out of $totalFiles files found."
